...
@section('maincontent')
                ...
                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Data Klasifikasi Tanah</li>
                        </ol>
                    </nav>
                </div>
            </div>
        ...
        <section class="section">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="card-title mb-0">Data Klasifikasi Tanah</h5>
                    <div class="d-flex">
                ...
                <div class="card-body">
                    ...
                    @else
                        <table class="table table-striped" id="table1">
                            ...
                            <tbody>
                                @foreach ($klasifikasitanah as $kt)
                                    <tr>
                                        ...
                                        <td>
                                            <button class="btn icon btn-primary" data-bs-toggle="modal"
                                                data-bs-target="#updateModal{{ $kt->id }}">
                                                <i class="bi bi-pencil"></i>
                                            </button>
                                            <!-- Tombol Hapus dengan SweetAlert -->
                                            <button class="btn icon btn-danger delete-button"
                                                data-id="{{ $kt->id }}" data-nama="{{ $kt->jenistanah }}">
                                                <i class="bi bi-trash3-fill"></i>
                                            </button>
                                        </td>
                                    </tr>
                                    @include('AdminPage.klasifikasitanahFeatures.update-modals', [
                                        'klasifikasitanah' => $kt
                                    ])
                                @endforeach
                            </tbody>
    ...
    <!-- Default size Modal -->
    @include('AdminPage.klasifikasitanahFeatures.create-modals')
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            deleteButtons.forEach(button => {
                button.addEventListener('click', function(e) {
                    e.preventDefault();
                    const klasifikasitanahID = this.getAttribute('data-id');
                    const klasifikasitanahNama = this.getAttribute('data-nama');

                    // SweetAlert untuk konfirmasi
                    Swal.fire({
                        title: 'Apakah Anda yakin?',
                        text: `Data ${klasifikasitanahNama} akan dihapus!`,
                        icon: 'warning',
                        ...
                        if (result.isConfirmed) {
                            ...
                            form.action = `/klasifikasitanahs/${klasifikasitanahID}`;
                            form.method = 'POST';
            ...
            pdfButton.addEventListener('click', function() {
                ...
                fetch('{{ route('klasifikasitanah.exportPDF') }}')
