...
@section('maincontent')
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex">
                        ...
                        <button type="button" class="btn btn-danger" id="exportPdfButton" data-bs-toggle="modal"
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <button class="btn icon btn-primary" data-bs-toggle="modal"
                                                data-bs-target="#updateModal{{ $desa->id }}">
                                                <i class="bi bi-pencil"></i> Edit Data
                                            </button>
                                            <!-- Tombol Hapus dengan SweetAlert -->
                                            <button class="btn icon btn-danger delete-button" data-id="{{ $desa->id }}"
                                                data-nama="{{ $desa->nama_desa }}">
                                                <i class="bi bi-trash3-fill"></i> Hapus Data
                                            </button>
                                        </td>
                                    </tr>
                                    @include('AdminPage.desaFeatures.update-modal', ['desa' => $desa])
                                @endforeach
                            </tbody>
                        </table>
                    @endif
    ...
    <!-- Default size Modal -->
    @include('AdminPage.desaFeatures.create-modal')
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            deleteButtons.forEach(button => {
                button.addEventListener('click', function(e) {
                    e.preventDefault();
                    const desaId = this.getAttribute('data-id');
                    const desaNama = this.getAttribute('data-nama');

                    // SweetAlert untuk konfirmasi
                    Swal.fire({
                        title: 'Apakah Anda yakin?',
                        text: `Data ${desaNama} akan dihapus!`,
                        icon: 'warning',
            ...
            pdfButton.addEventListener('click', function() {
                const desaTable = document.querySelector('#table1 tbody');
                if (!desaTable || desaTable.children.length === 0) {
                    Swal.fire({
                        icon: 'info',
                        title: 'Data Kosong',
