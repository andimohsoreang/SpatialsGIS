...
@section('maincontent')
                ...
                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Data SNI</li>
                        </ol>
                    </nav>
                </div>
            </div>
        ...
        <section class="section">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="card-title mb-0">Data SNI</h5>
                    <div class="d-flex">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <button class="btn icon btn-primary" data-bs-toggle="modal"
                                                data-bs-target="#updateModal{{ $sni->id }}">
                                                <i class="bi bi-pencil"></i> Edit Data
                                            </button>
                                            <!-- Tombol Hapus dengan SweetAlert -->
                                            <button class="btn icon btn-danger delete-button" data-id="{{ $sni->id }}"
                                                data-nama="{{ $sni->snis }}">
                                                <i class="bi bi-trash3-fill"></i> Hapus Data
                                            </button>
                                        </td>
                                    </tr>
                                    @include('AdminPage.snisFeatures.update-modals', ['snis' => $sni])
                                @endforeach
                            </tbody>
                        </table>
                    @endif
    ...
    <!-- Default size Modal -->
    @include('AdminPage.snisFeatures.create-modal')
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            deleteButtons.forEach(button => {
                button.addEventListener('click', function(e) {
                    e.preventDefault();
                    const sniId = this.getAttribute('data-id');
                    const sniNama = this.getAttribute('data-nama');

                    // SweetAlert untuk konfirmasi
                    Swal.fire({
                        title: 'Apakah Anda yakin?',
                        text: `Data ${sniNama} akan dihapus!`,
                        icon: 'warning',
